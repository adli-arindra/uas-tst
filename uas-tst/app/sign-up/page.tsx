...
import { auth, create_user } from "@/app/firebase/auth";
...
const SignUpPage = () => {
    ...
    const [passwordMatch, setPasswordMatch] = useState(true); // State for matching passwords
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (passwordValid !== "" || !passwordMatch) {
            console.error(passwordValid || "Passwords do not match");
            return;
        }
        try {
            const userinfo = await create_user(email, password);
                ...
                <form onSubmit={handleSubmit} className="space-y-4 text-gray-800">
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            required
                            className={`mt-1 block w-full px-4 py-2 border ${
                                !passwordMatch ? "border-error" : "border-gray-300"
                            } rounded-lg shadow-sm`}
                    ...
                    {/* Submit Button */}
                    {failedSignUp && <p className="text-error text-sm mt-1">E-mail is already in use!</p>}
                    ...
                    <div>
                        ...
                        <Link href="/sign-in">
                            <h1 className="inline-block pr-1 text-accent font-semibold">Sign In</h1>
                        </Link>
                    </div>
                </form>
            </div>
